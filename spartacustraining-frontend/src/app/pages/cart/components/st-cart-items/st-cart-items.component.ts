...
export class StCartItemsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _multiCartService: MultiCartService
  ) { }
  ...
  removeEntry(item: OrderEntry): void {
    if (this._selectiveCartService && this.options.isSaveForLater) {
      this._selectiveCartService.removeEntry(item);
    } else if (this.cartId && this._userId) {
      if (item.entryNumber != null) {
        this._multiCartService?.removeEntry(
          this._userId,
          this.cartId,
          item.entryNumber
        );
    ...
    return this.form.get(this.getControlName(item)).valueChanges.pipe(
      ...
      tap((value) => {
        if (item.updateable && value && !this.readonly) {
          if (this._selectiveCartService && this.options.isSaveForLater) {
            this._selectiveCartService.updateEntry(
              value.entryNumber,
              value.quantity
            );
          } else if (this.cartId && this._userId) {
            this._multiCartService?.updateEntry(
              this._userId,
              this.cartId,
              value.entryNumber,
              value.quantity
            ...
            this._activeCartService.updateEntry(
              value.entryNumber,
              value.quantity
            );
          }
